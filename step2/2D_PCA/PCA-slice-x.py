...
parser.add_argument('x_slice')
args = parser.parse_args()
x_slice = int(args.x_slice)

batch_size = 32
rs = 150
#ukbb_path_T1_slices = '/work/forkert_lab/erik/T1_cropped_slices/T1_cropped_slice_{}/train'.format(x_slice)
output_path = '/work/forkert_lab/erik/PCA/slices-x'
...
#data.shape
data = np.load(reshaped_path + '/reshaped_slice_{}.npy'.format(x_slice))
...
'''
with open(output_path + '/evecs_slice_{}.pkl'.format(x_slice), 'wb') as file:
    pickle.dump(evecs, file)

with open(output_path + '/evalues_slice_{}.pkl'.format(x_slice), 'wb') as file:
    pickle.dump(evalues, file)
'''

with open(output_path + f"/evalues_slice_{x_slice}.npy", 'wb') as f_variance:
    np.save(f_variance, np.array(pca.explained_variance_ratio_))

with open(output_path + f'/evecs_slice_{x_slice}.pkl', 'wb') as filefinal:
