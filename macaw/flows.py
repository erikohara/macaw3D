...
class Flow(nn.Module):
    ...
    def forward(self, x):
        ...
        z = torch.nan_to_num(x * torch.exp(s) + t)
    ...
    def backward(self, z):
        ...
        for i in range(self.dim):
            ...
            s = torch.nan_to_num(s)
            t = torch.nan_to_num(t)
            x[:, i] = torch.nan_to_num((z[:, i] - t[:, i]) * torch.exp(-s[:, i]))
            log_det += -s[:, i]
        return x, log_det
...
class NormalizingFlowModel(nn.Module):
        ...
        if type(self.priors) == list:
            ...
            for sl, dist in self.priors:
                if sl.start >= idx:
                    ...
                    '''
                    '''
                    if isinstance(dist, (Bernoulli, Categorical)):
                        data = torch.round(data)
                        data = torch.clamp(data, min=0, max=1)
                    prior_log_prob += dist.log_prob(data).view(x.size(0), -1).sum(1)
        else:
            prior_log_prob = self.priors.log_prob(zs[-1][:,idx]).view(x.size(0), -1).sum(1)
    ...
    def log_likelihood(self, x, idx=0):
        ...
        _, prior_logprob, log_det = self.forward(x,idx)
