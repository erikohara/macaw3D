...
parser.add_argument('cf_age_definition')
args = parser.parse_args()
cf_age_definition = int(args.cf_age_definition)
...
model_path = '/work/forkert_lab/erik/MACAW/models/PCA_sklearn'
reshaped_path = f'/work/forkert_lab/erik/MACAW/reshaped/2_5Z_{cf_age_definition}/slices-z'
output_image = f'/work/forkert_lab/erik/MACAW/cf_images/2_5Z_{cf_age_definition}_reverse'
...
sex_obs = sex
age_obs = age
cf_age_obs = np.full((len(age_obs)), cf_age_definition - min_age)
...
with open(cf_age_path + f'/cf_age_obs.pkl', 'wb') as cfagefile:
...
for z_slice in tqdm(range(slice_initial,slice_final)):
    ...
    #for ev in range(0,all_data_dim[order_slice],nevecs):
        ...
        try:
            ...
            '''
            macaw = torch.load(model_path + f'/slice_{z_slice}/macaw_ukbb_PCA_{ev}.pt', map_location=torch.device(config.device))
            macaw.model.to(config.device)
            macaw.model.flow.to(config.device)
            for each_flow in macaw.model.flow.flows:
                each_flow.to(config.device)
                each_flow.net.to(config.device)
            ...
            with torch.no_grad():
                # abduction:
                X_obs = np.hstack([np.array(sex_obs)[:,np.newaxis], np.array(cf_age_obs)[:,np.newaxis], encoded_obs])
...
def count_files_folder(path_file_to_count):
    number_files = []
    for file in os.listdir(path_file_to_count):
        file_name = file.split('_')[0]
        if file_name not in number_files:
            number_files.append(file_name)
    return len(number_files)
...
number_files = []
for file in os.listdir(output_image):
    file_name = file.split('_')[0]
    if file_name not in number_files:
        number_files.append(file_name)

print(f"Number of unique ID files is: {len(number_files)}")