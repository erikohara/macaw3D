...
#output_image = '/work/forkert_lab/erik/MACAW/cf_images/PCA3D_15000_new_zero_diff_2'
output_image = f'/scratch/{job_id}/PCA3D_15000_full_zero_diff'
    ...
    try:
        ...
        with torch.no_grad():
            ...
            if (len(non_zero_positions) > 0):
                #print(f"difference between forward and backward on evec {ev}:")
                #print(non_zero_positions)
                #print(diff)
                with open(output_image +f'/diff_{ev}.npy', 'wb') as f_diff:
                    np.save(f_diff, diff)
            '''
...
print(f"Number of unique ID files is: {number_files_output}")
