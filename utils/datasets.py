...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        item = torch.from_numpy(self.x[index]).to(self.device)
        return item
...
class MorphomnistDatasetOld(Dataset):
...
class UKBBT13DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #img_name = str(int(self.df.iloc[idx]['eid'])) + '.nii.gz'
        img_name = [filename for filename in os.listdir(self.img_dir) if filename.startswith(str(int(self.df.iloc[idx]['eid'])))][0]
        sample = nib.load(self.img_dir + '/' + img_name).get_fdata()
        if self.transform:
            sample = self.transform(sample)
        sample = np.expand_dims(sample, axis=0)
        return sample,self.df.iloc[idx]['Sex'], self.df.iloc[idx]['Age'], self.df.iloc[idx]['BMI'],img_name
