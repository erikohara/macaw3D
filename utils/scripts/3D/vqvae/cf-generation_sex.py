...
model_path = f"/work/forkert_lab/erik/MACAW/models/macaw_vqvae8_50nevecs_nan_to_num"
scalers_path = '/work/forkert_lab/erik/MACAW/scalers/macaw_vqvae8_50nevecs_nan_to_num'
reshaped_path = '/work/forkert_lab/erik/MACAW/reshaped/3D'
output_image = '/work/forkert_lab/erik/MACAW/cf_images/vqvae8_2_sex'
...
for channel in range(n_channels):
    ...
    #for ev in range(0,all_data_dim[order_slice],nevecs):
        encoded_obs =  encoded_data_all[:,channel,ev:ev+nevecs] 
        try:
            macaw = MACAW.MACAW(config)
            macaw.load_model(model_path + f'/c{channel}_macaw_ukbb_PCA3D_{ev}.pt',
            ...
            with torch.no_grad():
                ...
                if (len(cc_nan_places) > 0):
                    print(f"nan values produced on cc channel {channel} evec {ev}:")
                    print(cc_nan_places)
            ...
            print(e)
            print(f"File not finded: c{channel}_macaw_ukbb_PCA3D_{ev}.pt")
            ...
            print(f"Error in model c{channel}_macaw_ukbb_PCA3D_{ev}.pt")
            raise Exception('Just to stop and see', value_error)
        '''
        '''
        all_c_cf[:,channel,ev:ev+nevecs] = cc[:,ncauses:]
...
sex_cf = ['Male' if round(s) else 'Female' for s in cf_sex_obs]
