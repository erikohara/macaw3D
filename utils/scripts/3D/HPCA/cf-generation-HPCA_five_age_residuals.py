...
output_image = f"/work/forkert_lab/erik/MACAW/cf_images/HPCA_18000_five_residuals"
...
# Getting the residuals from 3D encoding
residual_3D = d_obs -decode(encode(d_obs,evecs[:ncomps]),evecs[:ncomps])
all_residuals = []
for z_slice in range(slice_initial,slice_final):
    order_slice = z_slice - slice_initial
    with open(residuals_path + '/residuals_slice_{}.npy'.format(z_slice),'rb') as f:
        slices_residuals = np.load(f)
    all_residuals.append(slices_residuals)
...
decoded_cf = decode(c_cf,evecs[:ncomps,:]) + residual_3D

all_decoded_cf = []
slice_order = 0
for ev in range(0,data_dim,1000):
    slice_decoded_cf = decode(decoded_cf[:,ev:ev+1000],all_evecs[slice_order][0:1000]) + all_residuals[order_slice]
...
for individual in range(nsamples):
    ...
    if new_number_files > number_files_output:
        ...
        print(f"File {df.loc[individual,'Subject']} not saved for some reason")
