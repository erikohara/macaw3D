...
model_path = f"/work/forkert_lab/erik/MACAW/models/PCA3D_10625"
reshaped_path = '/work/forkert_lab/erik/MACAW/reshaped/3D'
output_image = '/work/forkert_lab/erik/MACAW/cf_images/PCA3D_1500_many_ages'
cf_age_path = '/work/forkert_lab/erik/MACAW/2_5'
original_images = "/work/forkert_lab/erik/T1_warped"
scalers_path = '/work/forkert_lab/erik/MACAW/scalers/PCA3D_10625'
number_images = 13
cf_age_definition = [50,55,60,65,70,75]

# In[13]:
print("Starting code")

data = np.load(reshaped_path + '/reshaped_3D_test.npy')
...
#c_obs = causes[rands,:]
indexes = df.index[df['eid'].isin([2640932,
                                   2882482,
                                   3073918,
                                   3175799,
                                   3879651,
                                   ...
                                   5548258,
                                   5676439])].tolist()
data_obs = data[indexes,:]
sex_obs = sex[indexes]
age_obs = age[indexes]
random_eids = all_eid[indexes,0]
...
for cf_each_age in cf_age_definition:
    print(f"Starting with age {cf_each_age}")
    cf_vals = {1:cf_each_age - min_age}
    ...
    #for ev in range(0,all_data_dim[order_slice],nevecs):
        encoded_obs =  encode(data_obs,evecs3D[ev:ev+nevecs])   
        try:
            '''
